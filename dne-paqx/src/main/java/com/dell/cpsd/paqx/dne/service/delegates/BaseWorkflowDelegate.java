...
public abstract class BaseWorkflowDelegate implements JavaDelegate
{
    ...
    public void postExecute(final DelegateExecution delegateExecution)
    {
        ...
        {
            Map<String, List<String>> currentDelegateStatusMap = (Map<String, List<String>>) delegateExecution.getVariable(DELEGATE_STATUS_VARIABLE);
            if (currentDelegateStatusMap == null)
            {
                currentDelegateStatusMap = new HashMap<>();
            }

            List<String> currentDelegateStatus = currentDelegateStatusMap.get(delegateExecution.getProcessInstanceId());
            ...
            currentDelegateStatusMap.put(delegateExecution.getProcessInstanceId(), currentDelegateStatus);
            delegateExecution.setVariable(DELEGATE_STATUS_VARIABLE, currentDelegateStatusMap);
        }
    }
...
//        String updateString = dateString + statusString;
    ...
    public synchronized static void updateWorkflowErrorResult(boolean hasErrors, DelegateExecution delegateExecution) {
        WorkflowResult workflowResult = (WorkflowResult)delegateExecution.getVariable(WORKFLOW_RESULT);
        workflowResult.setHasErrors(hasErrors);
